...
//    fs::path bin;
    ...
    out << "//Generated by cpp.wrap" << std::endl;
    if (vm.count("template"))
    {
        fs::ifstream tpl{tpl_f};
        if (!tpl)
        {
            std::cerr << "Could not read " << tpl_f << std::endl;
            return 1;
        }
    ...
    if (!out)
    {
        std::cerr << "Writing " <<  output << " failed" << std::endl;
        return 1;
    }
    ...
    if (!wrp)
    {
        std::cerr << "Writing " <<  wrapper_out << " failed" << std::endl;
        return 1;
    }
